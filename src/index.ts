...
import { handleGenerateOllamaText, handleGenerateOpenAIText } from "./ipc/ai";
...
app.whenReady().then(() => {
  ipcMain.handle("files:list", handleRequestFiles);
  ...
  ipcMain.on("ollama:generateText", async (e, prompt) => {
    const parts = await handleGenerateOllamaText(e, prompt);
    for await (const part of parts) {
      e.sender.send("ollama:generatedText", part);
    }

    e.sender.send("ollama:generatedText", "--end--");
  });
});
